# Deploying Musical Dice Game

Target: **https://dicemusic.lab980.com** — served from the lab980 droplet (conventions in
...
deployment. Everything is driven by the operate CLI at `bin/dicemusic`.

> Why not `provision-site`? That script scaffolds proxy-shaped sites (an app on
> a local port). This site has no app, so `dicemusic setup` writes its own
> static vhost instead — same DNS/doctl, security-headers and certbot shape.

## One-time bring-up (on the droplet, as root)

```bash
git clone https://github.com/ivjames/dicemusic /var/www/dicemusic
ln -sf /var/www/dicemusic/bin/dicemusic /usr/local/bin/dicemusic
dicemusic setup
```

`dicemusic setup` is idempotent and does, in order:

1. **DNS** — `doctl` A record `dicemusic.lab980.com -> droplet IP` (skipped if it already
   exists; `--no-dns` to skip, `--ip` to override autodetect).
2. **nginx** — static vhost from `deploy/nginx.conf.template` installed as
   `/etc/nginx/sites-available/dicemusic.lab980.com`, symlinked into `sites-enabled/`,
...
3. **TLS** — waits for DNS to resolve, then
   `certbot --nginx -d dicemusic.lab980.com --redirect -n`. If DNS is still propagating it
...
```bash
dicemusic status              # HEAD commit, live probe, cert days
health-check --site dicemusic # the droplet-wide auditor also covers it
```

## Overrides

- `DICEMUSIC_FQDN` — serve under a different name (default `dicemusic.lab980.com`)
- `DICEMUSIC_BRANCH` — deploy a different branch (default `main`)
